...
class SteamLoginView(View):
    def get(self, request):
        ...
        redirect_url = f'https://steamcommunity.com/openid/login?openid.claimed_id=http://specs.openid.net/auth/2.0/identifier_select&openid.identity=http://specs.openid.net/auth/2.0/identifier_select&openid.mode=checkid_setup&openid.ns=http://specs.openid.net/auth/2.0&openid.realm={redirect_uri}&openid.return_to={redirect_uri}'
        ...
        if 'openid.identity' in request.GET:
            ...
            top_5_hours_last_week = [
                Game(
                    ...
                    (
                        0
                        if 'playtime_2weeks' not in i
                        else int(i['playtime_2weeks'] / 60)
                    ),
                )
                for i in sorted(
                    response['games'],
                    key=lambda x: (
                        0
                        if 'playtime_2weeks' not in x
                        else -1 * x['playtime_2weeks']
                    ),
                )[:5]
            ]
